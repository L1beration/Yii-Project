 ...
 *
 * The followings are the available columns in table 'tbl_students':
 * @property integer $id
...
class Students extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('student_name, class_id', 'required'),
			array('class_id', 'numerical', 'integerOnly'=>true),
			array('student_name', 'length', 'max'=>255),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, student_name, class_id', 'safe', 'on'=>'search'),
		);
	}
